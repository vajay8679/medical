            ...
            <div class="card-body formPaddingTab">
                    ...
                    <div v-for="data in datas" :class="data.field_type == 'text' || data.field_type == 'dropdownSelect'   || data.field_type == 'time' ||data.field_type == 'date' || data.field_type == 'number'  ? 'col-md-4 labelMargindown '   :  data.field_type == 'textarea' ? 'col-md-12 labelMargindown ' : ''">
                        ...
                        <div v-if="data.field_type == 'text'">
                            <div>
                                ...
                                <input :type="data.field_type" class="form-control" :id="data.input_name" aria-describedby="emailHelp" :placeholder="data.label_name" />
                                <span class="text-danger" v-for="(err, index) in error">
                                    <span class="text-danger" v-if="index == data.input_name" v-for="r in err">{{ r }}</span>
                                </span>
                            </div>
                        </div>
                        <div v-if="data.field_type == 'dropdownSelect'">
                            <div>
                                ...
                                <select class="form-select dropDownSize" aria-label="Default select example" :id="data.input_name">
                                    <option value="">
                                        {{ data.label_description }}
                                    </option>
                                    <option v-for="item in data.field_values" :value="item.value">
                                        {{ item.value }}
                                    </option>
                                </select>
                                <span class="text-danger" v-for="(err, index) in error">
                        ...
                        <div v-if="data.field_type == 'textarea'">
                            <div>
                                ...
                                <textarea :placeholder="data.label_description" class="form-control textAeraSize" :id="data.input_name" rows="3"></textarea>
                                <span class="text-danger" v-for="(err, index) in error">
                                    <span class="text-danger" v-if="index == data.input_name" v-for="r in err">{{ r }}</span>
                                </span>
                            </div>
                        ...
                        <div v-if="data.field_type == 'MultiLineText'">
                            <div>
                                ...
                                <textarea :placeholder="data.label_description" class="form-control" :id="data.input_name" rows="3"></textarea>
                                <span class="text-danger" v-for="(err, index) in error">
                                    <span class="text-danger" v-if="index == data.input_name" v-for="r in err">{{ r }}</span>
                                </span>
                            </div>
                        ...
                        <div v-if="data.field_type == 'time'">
                            <div>
                                ...
                                <select class="form-select" aria-label="Default select example" :id="data.input_name">
                                    <option value="">
                                        {{ data.label_description }}
                                    </option>
                        ...
                        <div v-if="data.field_type == 'checkbox'">
                            <div class="form-check">
                                <input class="form-check-input" :type="data.field_type" :id="data.input_name" name="myCheck" />
                                <div class="d-flex">
                                    <div>
                                        <label :for="data.input_name">{{
                                                data.label_name
                        ...
                        <div v-if="data.field_type == 'radio'">
                            <div class="d-flex">
                                ...
                                <div v-for="(
                                            item, index
                                        ) in data.field_values" :key="index" class="d-flex mx-2">
                                    <div>
                                        <!-- {{item.value}} -->
                                        <input class="form-check-input " type="radio" :name="data.input_name" :id="data.input_name" :value="item.value" />
                                    </div>
                                    <div class="mx-2">
                ...
                <div>
                    <div class="col-12 save-lead d-flex justify-content-center">
                        <button v-if="EditIds == null && oneValue != 0" type="submit" class="btn btn-primary" @click="formSubmit()">
                            Save Quote
                        </button>

                        <button v-else-if="EditIds != null && oneValue == 0" type="submit" class="btn btn-primary" @click="formSubmit()">
                            Save Quote
                        </button>
                        <button v-if="EditIds != null && oneValue != 0" type="submit" class="btn btn-primary" @click="formUpdate()">
                            Update Quote
                        </button>
                    </div>
                </div>
...
export default {
    ...
    methods: {
        ...
        getFormData() {
            ...
            axios
                .get(`${config.apiUrl}/api/form-data?obect_type_id=6`, {
                    headers: {
        ...
        getFormDataW(id) {
            let token = localStorage.getItem("token");
            let promise = axios.get(`${config.apiUrl}/api/quote-form/${id}`, {
                    headers: {
                        ...
                        if (retrnData.hasOwnProperty(key)) {
                            ...
                            if (idInput !== null) {
                                ...
                                if (value) {
                                    ...
                                    for (
                                        var i = 0; i < radioButtons.length; i++
                                    ) {
                                        if (
        ...
        formSubmit() {
            ...
            let token = localStorage.getItem("token");
            let promise = axios.post(config.apiUrl + "/api/quote-form", entry, {
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${token}`,
                },
            ...
            const consumer = () => {
                promise
                    .then((d) => {
                        ...
                        this.$router.replace("/quote-data");
                        this.getFormData();
                    })
                    .catch((e) => {
        ...
        formUpdate() {
            ...
            let promise = axios.put(
                `${config.apiUrl}/api/quote-form/${this.EditIds}`,
                entry, {
                    headers: {
                        "Content-Type": "application/json",
                        Authorization: `Bearer ${token}`,
